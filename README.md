...
| `tests/integration/`  | Integration test suite (Docker Compose + bash) |
...
| `server.auth_rate_limit`    | disabled | Optional  | Per-IP rate limit for auth failures.                                                       |
| `server.tls`                | disabled | Optional  | HTTPS with TLS 1.2+, mTLS via `client_ca_file`.                                            |
| `server.master_tls`         | disabled | Optional  | gRPC TLS. Blocks `insecure_skip_verify: true` (use `--allow-insecure-master` to override). |
| `server.rate_limit`         | disabled | Optional  | Per-IP token bucket rate limiter.                                                          |
| `server.skip_verify`        | `true`   | Optional  | Skip TLS verification for HTTP probes.                                                     |
| `server.run_cooldown`       | `10s`    | Optional  | Cooldown period for manual check runs.                                                     |
| `server.allowed_source_ips` | `[]`     | Optional  | Whitelist of allowed source IPs (CIDR). Empty = no restriction.                            |
| `server.audit`              | disabled | Optional  | Audit log per request.                                                                     |
| SSRF protection             |:        | Always on | Blocks `host` and `fallback_url` to private/link-local/metadata IPs.                       |
...
|----------|-------------|----------------------------------------------------------------|
| `type`   | `"none"`    | Auth mode: `none`, `api_key`, `bearer_token`.                  |
| `keys`   | `[]`        | Valid API keys or tokens. Required when auth is enabled.       |
| `header` | `X-API-Key` | Header name for `api_key`; `Authorization` for `bearer_token`. |

### Auth Rate Limit Sub-fields

| Field            | Default | Description                                   |
|------------------|---------|-----------------------------------------------|
| `enabled`        | `false` | Enable auth failure rate limiting.            |
| `max_failures`   | 10      | Max auth failures (401/403) before ban.       |
| `window_seconds` | 300     | Sliding window for counting failures (5 min). |
| `ban_seconds`    | 60      | Duration of ban after exceeding max failures. |
...
| `requests_per_second` |:       | Token refill rate. Required when enabled. |
| `burst`               |:       | Max burst size. Required when enabled.    |

### TLS Sub-fields

| Field            | Default | Description                                           |
|------------------|---------|-------------------------------------------------------|
| `cert_file`      |:       | Path to TLS certificate. Required with `key_file`.    |
| `key_file`       |:       | Path to TLS private key. Required with `cert_file`.   |
| `client_ca_file` |:       | CA certificate for mTLS. Enables client verification. |

### Master TLS Sub-fields

| Field                  | Default | Description                                                         |
|------------------------|---------|---------------------------------------------------------------------|
| `enabled`              | `false` | Enable TLS for gRPC master connection.                              |
| `insecure_skip_verify` | `false` | Skip cert verification (blocked without `--allow-insecure-master`). |
| `ca_file`              |:       | Custom CA certificate.                                              |
| `cert_file`            |:       | Client certificate for mTLS to master.                              |
| `key_file`             |:       | Client private key for mTLS to master.                              |
...
- **No retry / buffering:** If master is unreachable, submission is logged and silently dropped. ncapd does not
...
| `GET /metrics`          | Yes (when enabled) |
| `GET /results`          | Yes (when enabled) |
| `GET /results/{id}`     | Yes (when enabled) |
| `GET /checks`           | Yes (when enabled) |
| `POST /checks/{id}/run` | Yes (when enabled) |

Auth modes: `api_key` (custom header) or `bearer_token` (Authorization header). Missing/invalid → `401` or `403`.

When `server.auth_rate_limit` is enabled, repeated auth failures return `429` with `Retry-After`.
...
|-------|-----------------------|
| `200` | Check executed.       |
| `400` | Invalid check target. |
| `404` | Check not found.      |
| `429` | Check is in cooldown. |
...
Unit and integration tests (111 unit tests + 46 integration checks: auth, TLS/mTLS, rate limit, run cooldown, probes,
scheduler,
graceful shutdown).
