...
func Target(t core.Target, checkType core.Type) error {
	...
	case core.TypePortBlock, core.TypeSNIInspect, core.TypeTLSFP, core.TypeProtocolDetect, core.TypeThrottle:
		return validateHostPort(t, checkType)

	case core.TypeIPBlock:
		return validateIPBlock(t, checkType)

	case core.TypeDNSFilter:
		return validateDNSFilter(t, checkType)

	case core.TypeRSTInject:
		return validateHostPort(t, checkType)

	case core.TypeActiveProbe:
		return validateActiveProbe(t, checkType)

	default:
		return validateGeneric(t, checkType)
	}
}

func validateHostPort(t core.Target, checkType core.Type) error {
	if t.Host == "" {
	...
	if isBlockedForSSRF(t.Host) {
		return fmt.Errorf("host %q is blocked for SSRF prevention", t.Host)
	}
	if t.Port == 0 {
...
func validateIPBlock(t core.Target, checkType core.Type) error {
	if t.IP == "" {
...
func validateDNSFilter(t core.Target, checkType core.Type) error {
	if t.Host == "" {
		return fmt.Errorf("%w for check type %s", ErrHostRequired, checkType)
	}
	if err := Host(t.Host); err != nil {
...
func validateActiveProbe(t core.Target, checkType core.Type) error {
	if t.FallbackURL != "" {
...
func validateGeneric(t core.Target, checkType core.Type) error {
	if t.Host != "" {
		if err := Host(t.Host); err != nil {
			return fmt.Errorf("host: %w", err)
		}
...
func isBlockedForSSRF(host string) bool {
