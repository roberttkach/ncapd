...
func TestService_Run(t *testing.T) {
	...
	t.Run("successful execution", func(t *testing.T) {
		...
		if result.ID != "test-ok" {
			t.Errorf("expected ID 'test-ok', got %q", result.ID)
		}
		if result.Detail != "connected" {
			t.Errorf("expected detail 'connected', got %q", result.Detail)
		}
	})
	...
	t.Run("result is stored", func(t *testing.T) {
		...
		if stored.Status != OK {
			t.Errorf("expected stored status OK, got %s", stored.Status)
		}
	})
...
func TestService_ConcurrentAccess(t *testing.T) {
	...
	results := svc.AllResults()
	if len(results) != 100 {
		t.Errorf("expected 100 results after concurrent access, got %d", len(results))
	}
}
