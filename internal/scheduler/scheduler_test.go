...
func TestScheduler_ExecuteJob(t *testing.T) {
	t.Run("successful execution logs and submits", func(t *testing.T) {
		...
		if sub.CallCount() != 1 {
			t.Errorf("expected 1 submit call, got %d", sub.CallCount())
		}
		...
		if last.Status != core.OK {
			t.Errorf("expected submitted status OK, got %s", last.Status)
		}
	})
	...
	t.Run("error result submitted", func(t *testing.T) {
		...
		if last.Status != core.Error {
			t.Errorf("expected submitted status Error, got %s", last.Status)
		}
	})
...
func TestScheduler_StartAndScheduledJob(t *testing.T) {
	...
	if sub.CallCount() < 1 {
		t.Errorf("expected at least 1 submit call, got %d", sub.CallCount())
	}
}
