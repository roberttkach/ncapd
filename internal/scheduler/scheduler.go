...
func (s *Scheduler) Shutdown() context.Context {
	...
	cronCtx := s.cron.Stop()
	// Wait for running cron jobs to finish. They may spawn async submit
	// goroutines (tracked via submitWg), so we must wait for jobs first,
	// then for the submits they launched.
	<-cronCtx.Done()
	s.submitWg.Wait()
	return cronCtx
}
...
func (s *Scheduler) executeJob(req core.Request) {
	result := s.exec.Run(context.Background(), req)
	...
	// Submit result to master asynchronously; fire-and-forget via context.Background().
	// Goroutine is tracked via submitWg; Shutdown waits for all pending submits to complete.
	if s.sub != nil {
		s.submitWg.Add(1)
		go func() {
			defer s.submitWg.Done()
			if err := s.sub.Submit(context.Background(), result); err != nil {
