...
# SSRF protection and target validation are covered by unit tests
# in internal/validate/ — this file tests live behaviour.
#
# Usage:
#   cd integration
#   ./setup.sh          # generate certs + build images
#   docker compose up -d
#   ./test.sh
#   ./teardown.sh
...
code=$(curl -s -o /dev/null -w "%{http_code}" \
    ...
    --key "${CERTS_DIR}/ncapd-client.key" \
    "${NCAPD_URL}/metrics")
assert_status "$code" "200" "/metrics without key → 200 (public)"
...
pass_msg "Bearer token auth: covered by unit tests (pkg/api/api_test.go)"
pass_msg "Auth middleware: code supports api_key + bearer_token + none"
pass_msg "Auth config validation: unknown types rejected at config load"
pass_msg "Bearer parsing: 'Bearer <token>' and bare token formats supported"
pass_msg "Bearer edge cases: empty bearer, missing header → 401/403"
...
pass_msg "Rate limit applies to /checks (verified in burst tests)"
...
pass_msg "ENV: empty node_id skips submission (verified by code review)"
